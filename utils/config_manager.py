...
class WordlistsConfig:
    ...
    directories: str = field(
        default_factory=lambda: _get_platform_wordlist_path(
            "directory-list-2.3-medium.txt", "dirbuster"
        )
    )
    passwords: str = field(default_factory=lambda: _get_platform_wordlist_path("rockyou.txt"))
    usernames: str = field(
        default_factory=lambda: _get_platform_wordlist_path(
            "top-usernames-shortlist.txt", "usernames"
        )
    )
    subdomains: str = field(
        default_factory=lambda: _get_platform_wordlist_path(
            "subdomains-top1million-5000.txt", "dns"
        )
    )
